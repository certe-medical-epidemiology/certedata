...
#' confirmation.
#'
#' @inheritParams certedata_deps
#' @export
#' @examples
#' \dontrun{
#' certedata_update()
#' }
certedata_update <- function(recursive = FALSE, repos = getOption("repos")) {

  deps <- certedata_deps(recursive, repos)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All certedata packages up-to-date")
    return(invisible())
  }
